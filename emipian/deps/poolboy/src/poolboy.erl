...
%%	?ERROR_MSG("PoolBoy start_link :~p,~p~n",  [PoolArgs,WorkerArgs]),
...
init([{checklink, CheckLink} | Rest], WorkerArgs, State) ->
    init(Rest, WorkerArgs, State#state{checklink = CheckLink});
...
handle_cast({init}, State) ->
  #state{size = Size, supervisor = Sup} = State,
  Workers = prepopulate(Size, Sup),
	...
	%%	[fail | Left] ->
        ...
        [Pid | Left] ->
			if 
				...
				true -> 
				       case  CheckLink(Pid) of
						true ->
                         Ref = erlang:monitor(process, FromPid),
                        true = ets:insert(Monitors, {Pid, Ref}),
                         {reply, Pid, State#state{workers = Left}};
						 _->  
%%		                    ?ERROR_MSG("PoolBoy Check out Exit:~p~n",  [Pid]),
							exit(Pid,resume),
				            Work = new_worker(Sup, FromPid),
				            case Work of
				            {Pid0, Ref0}->
				              true = ets:insert(Monitors, {Pid0, Ref0}),
				              {reply, Pid0, State#state{workers = Left}};
				            fail->
				              {reply, fail, State#state{workers = Left}}
				            end
					%%	   {Pid1, Ref} = new_worker(Sup, FromPid),
					%%	   Pid1
						end
			end;		
...
handle_info({'EXIT', Pid, resume}, State) -> 
%%	?ERROR_MSG("PoolBoy EXIT1 :~n",  []),
	{noreply, State};
                ...
                true ->
			        ...
			        case W0 of
					 fail->
                      {noreply, State};
				     _-> 		
                     {noreply, State#state{workers = [W0| W]}}
                    end;  
                false ->
                    {noreply, State}
            end
...
%%		?ERROR_MSG("PoolBoy dismiss_worker :~n",  []),
...
handle_worker_exit(Pid, State) ->
    ...
    case queue:out(State#state.waiting) of
        {{value, {From, Ref}}, LeftWaiting} ->
			...
			_->
            gen_server:reply(From, fail),State#state{waiting = LeftWaiting}
           end;	
        {empty, Empty} when Overflow > 0 ->
