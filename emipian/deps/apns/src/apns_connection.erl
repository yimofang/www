...
-export([start_link/1, start_link/2, init/1, handle_call/3, handle_cast/2,
         handle_info/2, terminate/2,checkapns/1, code_change/3]).
...
send_message(ConnId, Msg) ->
	error_logger:info_msg("APNS cast ~p~n",
                         [ConnId]),
...
handle_call(Msg, From ,State) when is_record(Msg, apns_msg) ->
  Socket = State#state.out_socket,
     error_logger:info_msg("ANPS Calling ~n",
                         []),
  ...
  try
	  ...
	  case send_payload(
	    ...
	    ok ->
	      {noreply, State};
	    {error, Reason} ->
		     error_logger:info_msg("ANPS Calling Error ~p ~n",
                         [Reason]),
              Payload = build_payload(Msg),
...
handle_cast(Msg, State) when is_record(Msg, apns_msg) ->
  Socket = State#state.out_socket,
  Payload = build_payload(Msg),
      error_logger:info_msg("APNS handle_cast ~n",
                         []),
 ...
 try
  ...
  case send_payload(
    ...
    {error, Reason} ->
      apns_queue:fail(State#state.queue, Msg#apns_msg.id),
%%	 {stop, State#state{out_socket=undefined}}
      {stop, {error, Reason}, State}
  end
  catch
	  _:_->{noreply, State}
...
build_payload(Params, Extra, Content_Available) ->
      { Y} = do_build_payload(Params, Content_Available),
       X = [{"aps",dict:from_list(Y)} | Extra],
    Z =dict:from_list(X),
	Y1 =  rfc4627:encode(Z),
	list_to_binary(Y1).
...
do_build_payload([{Key, Value} | Params], Payload) ->
  case Value of
    Value when is_list(Value); is_binary(Value) ->
      do_build_payload(
        Params,
        [ {binary_to_list(atom_to_binary(Key, utf8)), unicode:characters_to_binary(Value)}
        | Payload]);
    Value when is_integer(Value) ->
      do_build_payload(Params, [{binary_to_list(atom_to_binary(Key, utf8)), Value} | Payload]);
    #loc_alert{action = Action,
      ...
      Json = {case Body of
                ...
                [{<<"loc-key">>, unicode:characters_to_binary(LocKey,utf8)},
                 {<<"loc-args">>,
                    lists:map(fun unicode:characters_to_binary/1, Args)}
                ]},
	  {Js} =Json,
      do_build_payload(Params, [{binary_to_list(atom_to_binary(Key, utf8)), dict:from_list(Js)} | Payload]);
    _ ->
...
checkapns(ConnId) ->
    case catch gen_server:call(ConnId, check,?TIMEOUT) of
      apnsok -> true;
		_->false
    end.
