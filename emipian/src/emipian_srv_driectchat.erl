%% @author hyf
%% @doc @todo Add description to emipian_srv_driectchat.


-module(emipian_srv_driectchat).
...
process(SenderSession,{Action,StampTime,MsgID,Content,MsgTime}) ->
    spawn(?MODULE,handle_action,[SenderSession,{Action,StampTime,MsgID,Content,MsgTime}]). 



handle_action(SenderSession,{Action,StampTime,MsgID,Content,MsgTime}) -> 
	try
     send(SenderSession,{Action,StampTime,MsgID,Content,MsgTime})
	after
	 exit(normal)   
	end.

processsrv(SenderSession,{Action,StampTime,MsgID,Content,MsgTime})->
  	send(SenderSession,{Action,StampTime,MsgID,Content,MsgTime}).
...
send(SenderSession,{Action,StampTime,MsgID,Content,MsgTime}
    ...
    S ="CALL c_sp_GetDirectUserID(\""++binary_to_list(SenderUserID)
          ++"\""++",@AReturnCode);"++
          "SELECT @AReturnCode", 
	...
	case  ReturnCodeCuror of
		not_found -> 
			{ActionResult,ToDabase}  = mod_action_directchat:get_resultparam(Action,-1,
		...
		_->
          ...
          if
			  RetunCode<0 ->
			    {ActionResult,ToDabase}  = mod_action_directchat:get_resultparam(Action,RetunCode,
														StampTime,MsgID,MsgTime,{}),
                MsgPid!{result,Action,MsgID,{ok,ActionResult,ToDabase}};
	          ...
	          true->		  
		         ...
		         Index22  = mysql_op:get_fieldIndex(DataCuror2, <<"fsSender101">>),
				  ...
				  AddtionInfo = 
                   ...
                   {ActionResult,ToDabase} = mod_action_directchat:get_resultparam(Action,0,StampTime,MsgID,MsgTime,AddtionInfo),
                    MsgPid!{result,Action,MsgID,{ok,ActionResult,ToDabase}},
...
sendnext(SenderSession,DataCuror,Indexes,MsgID,Content,MsgTime,AddtionInfo) ->
	...
	UserAddtionInfo = {},
  	 mod_action_directchat:sendmsg_to_user(SenderSession, MsgID, 
