%% @author hyf
%% @doc @todo Add description to mod_action_directchat.


-module(mod_action_directchat).
%% @author hyf
...
-define(AC_CS_DIRECTCHAT, 31007).
-define(AC_SC_DIRECTCHAT_R, 61007).
 
-define(AC_SC_DIRECTCHAT, 21010).
...
get_sendparam_fromfields(_Action,StampTime,MsgID,SendTime,Result)->
    ...
    {ok,{?AC_SC_DIRECTCHAT_R,Code,ReParam}}.
...
send_msg(SenderSession,Action,StampTime,MsgID,Content,MsgTime)->
	...
	emipian_msg_log:save_userreceiver(MsgID, SenderUserID,?CHATSTATUS_ONLY_REC,SenderUserID,MsgTime,{},?CHATSTYPE_DIRECT),
	emipian_srv_driectchat:processsrv
...
sendmsg_to_user(SenderSession, MsgID, Content, MsgTime, ReceiveUserID, 
  ...
  case save_userreceiver(MsgID,ReceiveUserID,Status,SenderUserID,MsgTime,UserAddtionInfo,?CHATSTYPE_DIRECT) of
	  {duplicate,_}->ok;
	  {ok,SynNo} ->
		ChatObj =1,
		Data =  get_sendmessagedata(MsgID,SenderUserID,UserType,Content,SynNo,
     ...
     #session{selfpid=PID,status =Status,sessionid =SessionID,userid=UserID} = Session,
      ?INFO_MSG("mod_action_groupchat Send:Msgid:~p,userid:~p,SelfPid:~p,Status:~p,SessionID:~p ~n", [MsgID,UserID,PID,Status,SessionID]),
	   ...
	   if 
		 ...
		 try	 
           PID ! {msg,?AC_SC_DIRECTCHAT,MsgID,SynNo,Data}
         catch
			_:_->ok 
         end;
           true->ok
...
get_resultparam(Action,Code,StampTime,MsgID,MsgTime,AddtionInfo)->
	...
	MsgID1 = emipian_util:str_to_binayid(MsgID),
   Return = {?AC_SC_DIRECTCHAT_R,Code,<<StampTime:64/little,MsgID1/binary,MsgTime:64/little>>},
   ToDataBase = 	
   if
	 Code=:=0 ->
   	 {action,?AC_SC_DIRECTCHAT_R,code,Code,addtioninfo,AddtionInfo};
   	 true->{action,?AC_SC_DIRECTCHAT_R,code,Code}
   end,
   {Return,ToDataBase}.
