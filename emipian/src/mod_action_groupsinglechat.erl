%% @author hyf
%% @doc @todo Add description to mod_action_groupsinglechat.


-module(mod_action_groupsinglechat).
...
-define(AC_CS_GROUPSINGLECHAT, 31005).
-define(AC_CS_GROUPSINGLECHAT_R, 61005).
 
-define(AC_SC_GROUPSINGLECHAT, 21008).
...
process_action(MsgID,Session,_Action,Param)-> 
      ...
      case Result of
		 cmderror -> cmderror;
		 {ok,StampTime,ReceiverLen,Receivers,_,Content,GroupID}->
			 ...
			 {ok,Receivers1,_} -> 
		      {FailReceivers,SMSReceivers,SucessInfos} = 
				  sendmsg_to_users(SessionID,AppCode,MsgID,SenderUserID,Content,0,MsgTime,
								   Receivers1,GroupID,[],[],[]),
			  {Reuslt,ToDataBase} = get_resultparam(?AC_CS_GROUPSINGLECHAT,?EC_SUCCESS,StampTime,MsgID,MsgTime,
										FailReceivers,SMSReceivers,SucessInfos),
		      {ok,Reuslt,ToDataBase};
             _->
			   {Reuslt,ToDataBase} = get_resultparam(?AC_CS_GROUPSINGLECHAT,?EC_CMDERROR,StampTime,MsgID,MsgTime,[],[],[]),
			  {ok,Reuslt,ToDataBase}
           end;
		 true->
	          {Reuslt,ToDataBase} = get_resultparam(?AC_CS_GROUPSINGLECHAT,?EC_CMDERROR,StampTime,MsgID,MsgTime,[],[],[]),
...
get_sendparam_fromfields(_Action,StampTime,MsgID,SendTime,Result)->
    ...
    {ok,{?AC_CS_GROUPSINGLECHAT_R,Code,ReParam}}.
...
get_sendfields_fromparam(_,Param)->
	case parse_param(Param) of
	   cmderror->cmderror;
	   {ok,StampTime,ReceiverLen,Receivers,ContentLen,Content,GroupID}->
	   {stamptime,StampTime,receiverlen,ReceiverLen,receivers,Receivers,contentlen,ContentLen,content,Content,groupid,GroupID}
     end.
...
sendmsg_to_terminal(_Action,MsgID,SynNo,Param,Session,SenderUserID,MsgTime,ChatObj,_)->
   ...
   #session{userid =ReceiverUserID} = Session, 
	 ...
	 UserAddtionInfo = emipian_msg_log:get_user_addtioninfo(MsgID, ReceiverUserID),
 	  Data =  get_sendmessagedata(MsgID,SenderUserID,Content,0,SynNo,ChatObj,MsgTime,GroupID,UserAddtionInfo),
			  ...
			  if
				 ...
				 true->
					  ...
					  if 
						 ...
						 true->
 		                     <<Content:ContentLen/binary,GroupID/binary>> = Rest0,
							 
							 {ok,StampTime,ReceiverLen,Receivers,ContentLen,Content,emipian_util:binary_to_str(GroupID)}
                       end
			  end		 
  end.
...
get_resultparam(Action,Code,StampTime,MsgID,MsgTime,FailedReceivers,SMReceivers,SucessInfos)->
   ...
   MsgID1 = emipian_util:str_to_binayid(MsgID),
   Return = {?AC_CS_GROUPSINGLECHAT_R,Code,<<StampTime:64/little,MsgID1/binary,MsgTime:64/little,FSize:32/little,SFailReceiver/binary,
	  SSize:32/little,SSMReceiver/binary,SInfoSize:32/little,SSucessInfo/binary>>},
   ToDataBase = 	
   if
	 Code=:=0 ->
   	 {action,?AC_CS_GROUPSINGLECHAT_R,code,Code,param,{failreceiver,SFailReceiver,smsreceiver,SSMReceiver,sucessinfo,SSucessInfo}};
   	 true->{action,?AC_CS_GROUPSINGLECHAT_R,code,Code}
   end, 
   {Return,ToDataBase}.
...
get_sendmessagedata(MsgID,SenderUserID,Content,ChatType,SynNo,ChatObj,
					MsgTime,GroupID,UserAddtionInfo)->
  ...
  <<MsgID1:40/binary,SynNo:32/little,ChatType:8/little,SenderUserID1:40/binary,MsgTime:64/little, 
	ChatObj:8/little,ContentLength:32/little, Content/binary,GroupID1:40/binary,AddtionInfoSize:32/little,AddtionInfoJson/binary>>. 

sendmsg_to_user(SessionID,MsgID,SenderUserID,Content,ChatType,MsgTime,ReceiveUserID,GroupID,UserAddtionInfo)->
  Status = if 
  ...
  case save_userreceiver(MsgID,ReceiveUserID,Status,SenderUserID,MsgTime,UserAddtionInfo,?CHATSTYPE_SINGLE) of
	  ...
	  {ok,SynNo} ->
		ChatObj =1,
		Data =  get_sendmessagedata(MsgID,SenderUserID,Content,ChatType,SynNo,ChatObj,MsgTime,GroupID,UserAddtionInfo),
...
sendmsg_to_terminal(MsgID,SynNo,SenderUserID,Session,Data,Content,UserAddtionInfo)->
     #session{selfpid=PID,status =Status,sessionid =SessionID,userid=UserID} = Session,
      ?INFO_MSG("mod_action_groupsinglechat Send:~p,SelfPid:~p,Status:~p,SessionID:~p ~n", [UserID,PID,Status,SessionID]),
  
	   if 
		 Status =:=?STATUS_ONLINE ->
         emipian_route:sendmsg(Session,{msg,?AC_SC_GROUPSINGLECHAT,MsgID,SynNo,Data});
...
sendmsg_to_users(SessionID,AppCode,MsgID,SenderUserID,Content,ChatType,MsgTime,
				 [Receiver|RestUsers],GroupID,FailReceivers,SMReceivers,SucessInfos)->
	ReceiverUserInfo = emipian_mysqldb:getuserinfoforgroup(SenderUserID,Receiver,GroupID, AppCode),
    if 
		  is_integer(ReceiverUserInfo) -> 
		  FailReceivers0 = FailReceivers++[Receiver],
   	      sendmsg_to_users (SessionID,AppCode,MsgID,SenderUserID,Content,ChatType,MsgTime,RestUsers,GroupID,FailReceivers0,SMReceivers,SucessInfos);
		true->
          ...
          sendmsg_to_user(SessionID,MsgID,SenderUserID,Content,ChatType,MsgTime,ReceiverUserID,GroupID,UserAddtionInfo),
	      sendmsg_to_users (SessionID,AppCode,MsgID,SenderUserID,Content,ChatType,MsgTime,RestUsers,GroupID,FailReceivers,SMReceivers,SucessInfos1)
   end.
