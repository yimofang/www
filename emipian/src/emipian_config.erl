%% @author hyf
%% @doc @todo Add description to emipian_config.


-module(emipian_config).
...
start() ->
    ...
    Config = get_emipian_config_path(),
    ...
    SharedKey = case erlang:get_cookie() of
                    nocookie ->
                        erlang:md5(randoms:get_string());
                    Cookie ->
                        erlang:md5(jlib:atom_to_binary(Cookie))
                end,
    State1 = set_option({node_start, global}, UnixTime, State),
    State2 = set_option({shared_key, global}, SharedKey, State1),
    set_opts(State2).

%% @doc Get the filename of the emipian configuration file.
%% The filename can be specified with: erl -config "/path/to/emipian.yml".
%% It can also be specified with the environtment variable emipian_CONFIG_PATH.
%% If not specified, the default value 'emipian.yml' is assumed.
%% @spec () -> string()
get_emipian_config_path() ->
    case application:get_env(config) of
	{ok, Path} -> Path;
	undefined ->
	    case os:getenv("EMIPIAN_CONFIG_PATH") of
		false ->
		    ?CONFIG_PATH;
		Path ->
		    Path
...
%% @doc Read the emipian configuration file.
...
%% @doc Read an emipian configuration file and return the terms.
%% Input is an absolute or relative path to an emipian config file.
...
%% Input is an absolute or relative path to an emipian configuration file.
...
get_absolute_path(File) ->
	...
	absolute ->
	    File;
	relative ->
	    Config_path = get_emipian_config_path(),
...
describe_config_problem(Filename, Reason) ->
    Text1 = lists:flatten("Problem loading emipian config file " ++ Filename),
...
%% If emipian isn't yet running in this node, then halt the node
exit_or_halt(ExitText) ->
    case [Vsn || {emipian, _Desc, Vsn} <- application:which_applications()] of
	[] ->
	    timer:sleep(1000),
...
get_version() ->
    list_to_binary(element(2, application:get_key(emipian, vsn))).
...
transform_terms(Terms) ->
    %% We could check all emipian beams, but this
