%% @author hyf
%% @doc @todo Add description to mod_action_singlechat.

 
-module(mod_action_singlechat).
...
-include("logger.hrl").
-include("macro.hrl").
-define(AC_CS_SINGLECHAT, 31001).
-define(AC_SC_SINGLECHAT_R, 61001).
 
-define(AC_SC_SINGLECHAT, 21001).
...
sendmsg_to_terminal(_Action,MsgID,SynNo,Param,Session,SenderUserID,MsgTime,ChatObj,_)->
	...
	#session{userid =ReceiverUserID} = Session, 
	 ...
	 UserAddtionInfo = 
      if ChatObj=:=1->emipian_msg_log:get_user_addtioninfo(MsgID, ReceiverUserID) ;
	  true-> <<"">>
      end,
...
sendmsg_to_user(SessionID,MsgID,SenderUserID,Content,ChatType,MsgTime,ReceiveUserID,UserAddtionInfo)->
  ...
  case save_userreceiver(MsgID,ReceiveUserID,Status,SenderUserID,MsgTime,UserAddtionInfo,?CHATSTYPE_SINGLE) of
	  ...
	  {ok,SynNo} ->
		ChatObj =  
			if ReceiveUserID=:=SenderUserID ->3;
              true-> 1
			 end,  
...
sendmsg_to_terminal(MsgID,SynNo,SenderUserID,Session,Data,Content,UserAddtionInfo)->
     #session{selfpid=PID,status =Status,sessionid =SessionID,userid=UserID} = Session,
      ?INFO_MSG("mod_action_singlechat Send MsgID:~p,UserID:~p,SelfPid:~p,Status:~p,SessionID:~p ~n", [MsgID,UserID,PID,Status,SessionID]),
  
	   if 
		 Status =:=?STATUS_ONLINE ->
         ?INFO_MSG("mod_action_singlechat routemsg~n", []),
...
sendmsg_to_users(SessionID,AppCode,MsgID,SenderUserID,Content,ChatType,MsgTime,
	...
	ReceiverUserInfo = emipian_mysqldb:getuserinfoforaid(SenderUserID,Receiver, AppCode),
    if 
		...
		true->
     	...
     	%%  UserAddtionInfo = {cardid,SenderCardID,s101,Sender101},
     	  UserAddtionInfo = {s101,Sender101},

		%%  SucessInfo ={aid,Receiver,userid,ReceiverUserID,cardid,ReceiverCardID,s101,Receiver101},
		  SucessInfo ={aid,Receiver,userid,ReceiverUserID,nickname,Receiver101},
...
%%	Dict1 =  dict:store("cardid", CardID,Dict),
	Dict2 = dict:store("nickname", S101,Dict),
