...
start() ->
    ets:new(emipian_modules,
	    [named_table, public,
	     {keypos, #emipian_module.module_host}]),
    ok.
...
start_module(Host, Module, Opts) ->
    ets:insert(emipian_modules,
	       #emipian_module{module_host = {Module, Host},
				opts = Opts}),
    try Module:start(Host, Opts) catch
      Class:Reason ->
	  ets:delete(emipian_modules, {Module, Host}),
	  ErrorText =
...
stop_module_keep_config(Host, Module) ->
    case catch Module:stop(Host) of
	  ...
	  ets:delete(emipian_modules, {Module, Host}),
	  ok;
      _ -> ets:delete(emipian_modules, {Module, Host}), ok
    end.
...
get_module_opt(Host, Module, Opt, F, Default) ->
    OptsList = ets:lookup(emipian_modules, {Module, Host}),
    case OptsList of
      [] -> Default;
      [#emipian_module{opts = Opts} | _] ->
	  get_opt(Opt, Opts, F, Default)
    end.
...
loaded_modules(Host) ->
    ets:select(emipian_modules,
	       [{#emipian_module{_ = '_', module_host = {'$1', Host}},
		 [], ['$1']}]).
...
is_loaded(Host, Module) ->
    ets:member(emipian_modules, {Module, Host}).
