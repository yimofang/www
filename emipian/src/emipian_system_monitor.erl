%% @author hyf
%% @doc @todo Add description to emipian_system_monitor.

 
-module(emipian_system_monitor).
...
init(Opts) ->
    ...
    lists:foreach(fun (Host) ->
			  emipian_hooks:add(local_send_to_resource_hook, Host,
...
detailed_info(Pid) ->
    case process_info(Pid, dictionary) of
	  ...
	  case lists:keysearch('$ancestors', 1, Dict) of
		...
		case Sup of
		  emipian_c2s_sup -> c2s_info(Pid);
		%%  emipian_s2s_out_sup -> s2s_out_info(Pid);
		%%  emipian_service_sup -> service_info(Pid);
