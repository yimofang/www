...
%% Added by vinoth on 18-03-2014 for stored procedures
		...
		{ok, Version} ->
			...
			[{_ResultType, _MySQLRes}] ->
			    Parent ! {mysql_conn, self(), ok},
                   EncodingList = atom_to_list(utf8),
				    do_query(Sock, RecvPid, LogFun,
					     "set names '"++EncodingList++"'",
					     Version,[]),				
...
loop(State) ->
	...
	{mysql_recv, _, closed, normal} ->
	    mysql:log(State#state.log_fun, normal, "mysql_conn: "
		      "Received receiver close signal, exiting.", []),
	    close_connection(State);
        Unknown ->
	    mysql:log(State#state.log_fun, error, "mysql_conn: "
...
get_query_response(LogFun, RecvPid, Version, Options) ->
	...
	{ok, <<Fieldcount:8, Rest/binary>>, _} ->
		...
		0 ->
		    %% No Tabular data
		    <<AffectedRows:8, _InsertId:8, Status/binary>> = Rest,
		    case Status of
				<<First:8, _Rest/binary>> when (First band ?SERVER_MORE_RESULTS_EXIST)== ?SERVER_MORE_RESULTS_EXIST ->
					[{updated, #mysql_result{affectedrows=AffectedRows}} | get_query_response(LogFun, RecvPid, Version, Options)];
				_ ->					
		...
		_ ->
			...
			{ok, Fields} ->
				...
				{ok, Rows, hasmore} ->
					[{data, #mysql_result{fieldinfo=Fields,
							 rows=Rows}} | get_query_response(LogFun, RecvPid, Version, Options)];	
				{error, Reason} ->
...
get_rows(N, LogFun, RecvPid, ResultType, Res) ->
	...
	{ok, Packet, _Num} ->
	    case Packet of
		<<254:8, _Warning:16, Status/binary>> ->
			case Status of
				<<First:8, _Second/binary>> 
				  when  (First band ?SERVER_MORE_RESULTS_EXIST)== ?SERVER_MORE_RESULTS_EXIST ->
					{ok, lists:reverse(Res), hasmore};
				_ ->					
					{ok, lists:reverse(Res)}
			end;		
		<<254:8, Rest/binary>> when size(Rest) < 8 ->
